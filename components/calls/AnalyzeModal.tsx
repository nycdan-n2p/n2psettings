...
function KpiCard({ label, value, sub, icon: Icon, color }: {
  ...
  return (
    <div className="bg-[#F9F9FB] rounded-[20px] p-4 flex items-start gap-3">
...
export function AnalyzeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-4 px-4 bg-black/40">
      <div className="modal-surface bg-white rounded-t-[34px] rounded-b-none shadow-2xl w-full max-w-4xl h-[calc(100vh-1rem)] flex flex-col">
        {/* Header */}
        <div className="modal-header flex items-center justify-between px-6 py-4 bg-white rounded-t-[34px]">
          <div>
        ...
        <div className="modal-body bg-white overflow-y-auto flex-1 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <KpiCard label="Total Talk Time" value={fmtSec(kpis.totalTalkSec)}
              ...
              icon={Mic} color="bg-gray-100 text-gray-600" />
          </div>

          {/* Stats row */}
          <div className="grid grid-cols-3 gap-2 text-center">
            {[
              { label: "Voicemail", value: `${kpis.voicemailPct}%`, color: "text-gray-900" },
              { label: "Blocked", value: `${kpis.blockedPct}%`, color: "text-gray-900" },
              { label: "Peak Hour", value: `${kpis.peakHour}:00`, color: "text-gray-900" },
            ].map(({ label, value, color }) => (
              <div key={label} className="bg-[#F9F9FB] rounded-[20px] py-3 px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="bg-[#F9F9FB] rounded-[20px] p-4">
              <h3 className="text-base font-semibold text-gray-900 mb-3">Result Breakdown</h3>
              <div className="space-y-2">
                {resultBreakdown.slice(0, 6).map(({ result, count }) => (
                  <div key={result} className="flex items-center gap-2">
                    <span className="text-xs text-gray-600 w-32 truncate">{result}</span>
                    <MiniBar value={count} max={maxResult} color="bg-gray-500" />
            ...
            {/* Hourly call pattern */}
            <div className="bg-[#F9F9FB] rounded-[20px] p-4">
              <h3 className="text-base font-semibold text-gray-900 mb-3">Calls by Hour</h3>
              <div className="flex items-end gap-0.5 h-20">
                {hourlyPattern.map(({ hour, calls }) => (
                  <div key={hour} className="flex-1 flex flex-col items-center gap-0.5">
                    <div
                      className={`w-full rounded-sm ${calls === Math.max(...hourlyPattern.map(h => h.calls)) ? "bg-gray-600" : "bg-gray-300"}`}
          ...
          {topAgents.length > 0 && (
            <div className="bg-[#F9F9FB] rounded-[20px] p-4">
              <h3 className="text-base font-semibold text-gray-900 mb-3">Top Agents by Talk Time</h3>
              <div className="space-y-2">
                {topAgents.slice(0, 7).map((agent) => (
                  <div key={agent.name} className="flex items-center gap-3">
                    <div className="w-28 text-xs text-gray-700 truncate shrink-0">{agent.name}</div>
                    <MiniBar value={agent.talkTimeSec} max={maxAgentTalk} color="bg-gray-500" />
                    <div className="ml-1 flex items-center justify-end gap-3 shrink-0">
                      <div className="text-xs text-gray-500 w-14 text-right">{fmtSec(agent.talkTimeSec)}</div>
                      <div className="text-xs text-gray-400 w-16 text-right">{agent.callCount} calls</div>
                      <span className="w-4 h-4 inline-flex items-center justify-center" title={agent.missedPct > 20 ? `${agent.missedPct}% missed` : undefined}>
                        {agent.missedPct > 20 ? (
          ...
          {/* Longest call */}
          {kpis.longestCall && (
            <div className="bg-[#F9F9FB] rounded-[20px] p-4 flex items-center gap-3">
          ...
          <div className="bg-[#F9F9FB] rounded-[20px] p-5">
